globals [
  ...
  external-field-increment ;; means that we can always instantly calculate
  temperature-increment    ;; the magnetization (which is the average spin)
]
...
to soft-setup ;;; no clear all
  ask patches [
    ifelse random 100 < probability-of-spin-up ;; determines the percentage of patches with spin = 1 at the beginning
      [ set spin  1 ]
      [ set spin -1 ]
    recolor ;; light blue: spin == 1; dark blue: spin == -1
  ]
  set sum-of-spins sum [ spin ] of patches
  set external-field 0
  set external-field-increment 0.001
  set temperature 10
  set temperature-increment -0.001
  reset-ticks
end
...
to go
  ...
  ;; behavior depending on the selected simulation
  if select-program = "simulate hysteresis" [
   simulate-hysteresis
  ]
  if select-program = "simulate pitchfork" [
   simulate-pitchfork
  ]
  if select-program = "simulate hysteresis temp 1" [
   simulate-hysteresis-temp-1
  ]
  update-plots       ;; unlike `tick`, `tick-advance` doesn't update the plots, so we need to do so explicitly
end

to simulate-pitchfork
  if ticks = 5000000 [
    set temperature 4.4
  ]
  if ticks = 10000000 [
...
to simulate-hysteresis
  set temperature 3.85 ;; I found this temperature to be somewhat of a sweetspot for a magnetic field that varies between 1 and -1
  if external-field <= -1   [
   set external-field-increment 0.001
  ]
  if external-field >= 1 [
  ...
  set external-field external-field + external-field-increment ;; external field strenght is gradually adjusted, but magnetization follows after hysteresis
end


to simulate-hysteresis-temp-1
  set temperature 1
  if ticks = 1000 [
    set external-field 1
  ]
  if (magnetization = 1) and (external-field >= -4) [ ;; compared with higher temp, a much larger magnetic field is required to change the stable state
   set external-field external-field - 0.02
  ]
  if (magnetization = -1) and (external-field <= 4) [
    set external-field external-field + 0.02
  ]
end
...
to update  ;; patch procedure
  ...
  ;; i.e., 1 -> -1: -2 = -2 * 1
  let Ediff (2 * spin * sum [ spin ] of neighbors) + (2 * external-field * spin) ;; 4 sorounding patches; added the term for the external field
  if (Ediff <= 0) or (temperature > 0 and (random-float 1.0 < exp ((- Ediff) / temperature))) [
    set spin (- spin) ;; flips 1 to -1 and -1 to 1
...
; Adjustments 2022 Luke Korthals
@#$#@#$#@
GRAPHICS-WINDOW
315
10
...
"" ""
PENS
"average spin" 1.0 2 -16777216 true "" "plotxy ticks magnetization"
"axis" 1.0 0 -14070903 true ";; draw a horizontal line to show the x axis\nauto-plot-off\nplotxy 0 0\nplotxy 1000000000000000 0\nauto-plot-on" ""
"external field" 1.0 0 -5298144 true "" "plotxy ticks external-field"
...
select-program
select-program
"free simulation" "simulate hysteresis" "simulate pitchfork" "simulate hysteresis temp 1"
0
...
995
151
Use the free simulation to play around. \n\nIf you use one of the other simulations, just press go and watch what happens
11
0.0
1
...
"" ""
PENS
"default" 1.0 1 -15575016 true "" "plotxy ticks temperature"
